...
Power_r<-function(D,k, alpha, beta,h0,hypothesis,location,scale,dff,model, 
  ...
  for (i in seq_along(Ns)) {
    r <- r_BF_bound_10(D,Ns[i],k, alpha, beta,h0,hypothesis,location,scale,dff,model)
    # Choose correct design prior:
    power.vals[i] <- if (de_an_prior == 1)
      r_TPE(r,Ns[i],k, alpha, beta,h0,hypothesis,location,scale,dff,model) else
        r_TPE(r,Ns[i],k_d, alpha_d, beta_d,h0,hypothesis,location_d,scale_d,dff_d,model_d)
  }
