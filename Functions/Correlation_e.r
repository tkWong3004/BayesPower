...
Power_re<-function(D,k, alpha, beta,h0,hypothesis,location,scale,dff,model, 
  ...
  power.vals <- numeric(length(Ns))
  
  
  for (i in seq_along(Ns)) {
    r <-  re_BF_bound_10(D,Ns[i],k,alpha, beta,h0,hypothesis,location,scale,dff,model,e)
    # Choose correct design prior:
    power.vals[i] <- if (de_an_prior == 1)
      re_TPE(r,Ns[i],k, alpha, beta,h0,hypothesis,location,scale,dff,model,e) else
        re_TPE(r,Ns[i],k_d, alpha_d, beta_d,h0,hypothesis,location_d,scale_d,dff_d,model_d,e)
  }
  
  plot(Ns, power.vals, type = "l",
       xlab = "Sample size",
