...
sourceCpp("Functions/grid.cpp", cacheDir = "tmp_cache")
sourceCpp("Functions/sum_rcpp.cpp", cacheDir = "tmp_cache")
...
source("Functions/proportions.r")


ui <- navbarPage(id = "id",
                 "\\(\\text{BayesPower}_{alpha}\\)",
  navbarMenu(
      ...
      sidebarLayout(
        ...
        mainPanel(
          navset_card_underline(
            ...
            nav_panel(id = "pt2", em("\\(\\text{Power Curve}\\)"), plotOutput("PCt2"))
          )
        )
      )
    )
...
tabPanel("\\(\\text{Correlation}\\)", withMathJax(),
         sidebarLayout(
           sidebarPanel(
             ...
             fluidRow(
               column(4,
                      sliderInput("h0pho", "\\(\\rho_0\\)", min = -1, max = 1, value = 0, step = 0.01, ticks = FALSE)
               ),
               column(4,
                      conditionalPanel(
             ...
             fluidRow(
               column(width = 4,
                      conditionalPanel(condition = "input.modelf == 1",
                                       sliderInput("rf", "\\(\\text{r scale:}\\)", min = 0, max = 3, value = 1, step = 0.01, ticks = FALSE)
                      )
               ),
               column(width = 4,
...
source("Server/Server_p2.r",local = T)
