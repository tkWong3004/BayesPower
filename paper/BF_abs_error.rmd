...
t1_TPE_abs_error <- function(t, df, prior_analysis, location, scale, dff, alternative ) {
  ...
  if (prior_analysis == "Point"){
    ...
    pro <- switch(alternative,
                 "two.sided"= BayesPower:::pnct(min(t), df, ncp) + (1 - BayesPower:::pnct(max(t), df, ncp)),
                 "greater" =1 - BayesPower:::pnct(t, df, ncp),
                 "less" =  BayesPower:::pnct(t, df, ncp))
    return(pro)
  }
 ...
 # integral, marginalized probability 
  int <- function(delta) {
    ...
    pro <- switch(alternative,
                  ...
                  "greater" = 1 - BayesPower:::pnct(t, df, delta * sqrt(df + 1)),
                  "less" = BayesPower:::pnct(t, df, delta * sqrt(df + 1))
    )
...
r_TPE_abs_error <-function(r,n,k, alpha, beta,h0,alternative,location,scale,dff,prior_analysis){
  ...
  # limits of the integral, the parameter space under h1
  bound  <- switch(alternative,
                   "greater" = c(a = h0, b = 1),
                   "less" = c(a = -1, b = h0),
                   "two.sided" = c(a = -1, b = 1)
  ...
  # normalization of the prior under h1
    ...
    switch(prior_analysis,
           "d_beta"   = BayesPower:::p_beta(bound[2], 1/k, 1/k,-1,1)-BayesPower:::p_beta(bound[1], 1/k,1/k,-1,1) ,
           "beta" = BayesPower:::p_beta(bound[2], alpha, beta,-1,1)-BayesPower:::p_beta(bound[1], alpha, beta,-1,1),
           "Moment"   = {BayesPower:::pmom(bound[2]-location, tau=scale^2)-BayesPower:::pmom(bound[1]-location, tau=scale^2)})
  }
