...
Users can replicate or extend these simulations using the scripts in this folder. Each script loads the required packages and defines custom functions for simulation. Toward the end of the script, users may adjust the input parameters as needed and run the simulation by executing the final lines of code. Although a simulation with as few as 1,000 iterations already produces results close to those obtained with our method (as shown below for some tests), we recommend that users employ a higher number of iterations for improved estimation based on the simulation.
...
### ANOVA
```{r , echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=6}
f_ver(iter=1000,D=3,p=3,k=4,dff=3,rscale=.18,f_m=.1,model="effectsize",
...
### One proportion
```{r , echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=6}
bin_ver(iter=1000,h0=.5, D=3,model="beta",  alpha=1, beta=1,location=.5,
        scale=1, hypothesis=">" ,model_d="Point",
        location_d=.5,k_d=1, alpha_d=1, beta_d=1,scale_d=1,de_an_prior=1,e=NULL)
```
...
```{r , echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=6}
p2_ver(iter=1000,D=3, a0=1, b0=1, a1=156, b1=339, a2=151, b2=339,
