...
Based on the simulated delta under H0 and H1, the prior predictive distributions of *t*-values are obtained for a vector of sample sizes. The four rates (true positive, false positive, true negative, and false negative) are calculated based on the frequency of *t*-values falling inside or outside the critical *t*-values, divided by the number of iterations. The numeric results are also obtained.
...
```{r}
 ...
 for (i in 1:length(sample_size)){
  ...
  suppressWarnings(
    if(is.null(ROPE)){
t.cri.h1 <- BayesPower:::t1_BF10_bound(threshold, df,
                                       prior_analysis, location, 
                                       scale, dff, alternative)
t.cri.h0 <- BayesPower:::t1_BF01_bound(threshold, df, 
                                       prior_analysis,location,
                                       scale, dff, alternative)
    } else{
      
      t.cri.h1 <- BayesPower:::t1e_BF10_bound(threshold, df,
                                              prior_analysis, 
                                              location, scale, dff, 
                                              alternative,ROPE)
      t.cri.h0 <- BayesPower:::t1e_BF01_bound(threshold, df, 
                                              prior_analysis, 
                                              location, scale, dff, 
                                              alternative,ROPE)
