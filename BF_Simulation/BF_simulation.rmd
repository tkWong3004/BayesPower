...
1. **Simulating cumulative probabilities**  
   A vector of cumulative probabilities of length $i$ is generated under the null hypothesis and the alternative hypothesis. These probabilities are then transformed into effect sizes based on the design prior.
...
Simulations are conducted for sample sizes ranging from 100 to 1000 in increments of 100.
...
#### Simulating t-values and the operational characteristics of the test
Based on the simulated delta under $\mathcal{H}_0$ and $\mathcal{H}_1$, the prior predictive distributions of the $t$-values are obtained for a vector of sample sizes. The four rates (true positive, false positive, true negative, and false negative) are calculated based on the frequency of $t$-values falling inside or outside the critical $t$-values, divided by the number of iterations. The numeric results are also obtained.
...
#### Simulating t-values and the operational characteristics of the test.
Based on the simulated delta under H0 and H1, the prior predictive distributions of *t*-values are obtained for a vector of sample sizes. The four rates (true positive, false positive, true negative, and false negative) are calculated based on the frequencies of *t*-values falling inside or outside the critical *t*-values, divided by the number of iterations. The numeric results are also obtained.
...
```{r, results='hide'}
  ...
  prior_design <- "beta"; alpha_d <- 2; beta_d <- 2; 
  location_d <- .3;  k_d <- 2; scale_d <- .3
  threshold <- 3
...
#### Simulating correlation and the operational characteristics of the test.
Based on the simulated rho under H0 and H1, the prior predictive distributions of sampled correlations are obtained for a vector of sample sizes. The four rates (true positive, false positive, true negative, and false negative) are calculated based on the frequencies of sampled correlations falling inside or outside the critical *r* values, divided by the number of iterations. The numeric results are also obtained.
...
#### Simulating f-values and the operational characteristics of the test.
Based on the simulated squared *f* under H0 and H1, the prior predictive distributions of *f*-values are obtained for a vector of sample sizes. The four rates (true positive, false positive, true negative, and false negative) are calculated based on the frequencies of the *f*-values falling inside or outside the critical *f*-values, divided by the number of iterations. The numeric results are also obtained.
...
```{r, results='hide'}
  ...
  prior_design <- "beta"
  alpha_d <- 1; beta_d <- 2; location_d <- .3; scale_d <- .1
  ROPE <- NULL
...
#### Simulating the number of success and the operational characteristics of the test.
Based on the simulated theta under H0 and H1, the prior predictive distributions of the number of successes are obtained for a vector of sample sizes. The four rates (true positive, false positive, true negative, and false negative) are calculated based on the frequencies of the Bayes factors. The numeric results are also obtained.
