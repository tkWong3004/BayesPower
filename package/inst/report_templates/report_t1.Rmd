...
cat(paste0(params$x$prior_analysis,"(location = ", params$x$location,", scale = ",params$x$scale)
    )
if (params$x$prior_analysis == "t-distribution"){
...
if (params$x$de_an_prior == 0) {
  ...
  if (params$x$prior_design == "Point") {
    cat(sprintf("Point(location = %s)  \n", params$x$location_d))
  } else {
    cat(sprintf("%s(location = %s, scale = %s", 
                params$x$prior_design, 
                params$x$location_d, 
                params$x$scale_d))
    
    if (params$x$prior_design == "t-distribution") {
      cat(sprintf(", df = %s)\n", params$x$dff_d))
    } else {
      cat(")\n")
    }
