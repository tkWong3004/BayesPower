...
#pragma omp parallel for collapse(2)
  ...
  if (model1 == "same") {
    ...
    for (int i = 0; i <= n1; ++i) {
      d_k1_dp[i] = lchoose_k1[i] + i * std::log(dp1) + (n1 - i) * std::log(1.0 - dp1);
    }
  }

  // Handle model2 adjustments
  if (model2 == "same") {
    d_k2_dp = d_k2;
  } else if (model2 == "beta") {
    for (int j = 0; j <= n2; ++j) {
    ...
    for (int j = 0; j <= n2; ++j) {
      d_k2_dp[j] = lchoose_k2[j] + j * std::log(dp2) + (n2 - j) * std::log(1.0 - dp2);
    }
  }
